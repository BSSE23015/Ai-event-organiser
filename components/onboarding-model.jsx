...
export default function OnboardingModal({ isOpen, onClose, onComplete }) {
  ...
  // 🇵🇰 Dynamically fetch Pakistan provinces
  const pakistanStates = useMemo(() => {
    return State.getStatesOfCountry("PK");
  }, []);

  // Dynamically fetch cities based on selected province
  const cities = useMemo(() => {
    if (!location.state) return [];

    const selected = pakistanStates.find((s) => s.name === location.state);

    if (!selected) return [];

    return City.getCitiesOfState("PK", selected.isoCode);
  }, [location.state, pakistanStates]);
